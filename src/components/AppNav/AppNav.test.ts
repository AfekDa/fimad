...
describe('AppNav', () => {
  ...
  it('renders implemented destinations as links', async () => {
    ...
    expect(body.querySelector('[data-nav-id="fanduel"]')).toHaveAttribute('href', '/fanduel')
  })
  ...
  it('renders every navigation destination as a link', async () => {
    const body = await renderAppNav()

    expect(body.querySelectorAll('a')).toHaveLength(5)
  })
